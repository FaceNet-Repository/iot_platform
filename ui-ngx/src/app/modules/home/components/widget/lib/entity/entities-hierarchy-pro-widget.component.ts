...
import { debounceTime, distinctUntilChanged, map, mergeMap, skip, startWith, takeUntil } from 'rxjs/operators';
...
import { EntityRelation, EntityRelationsQuery } from '@shared/models/relation.models';
import { AliasFilterType, RelationsQueryFilter } from '@shared/models/alias.models';
import { EntityFilter } from '@shared/models/query/query.models';
import { FormBuilder } from '@angular/forms';
import { forkJoin, Observable, of, Subject } from 'rxjs';
import { Asset, AssetInfo, Device, EntityId, PageData, PageLink } from '@app/shared/public-api';
...
@Component({
  selector: 'tb-entities-hierarchy-pro-widget',
  templateUrl: './entities-hierarchy-pro-widget.component.html',
  styleUrls: ['./entities-hierarchy-pro-widget.component.scss']
})
export class EntitiesHierarchyProWidgetComponent extends PageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  displayedColumns: string[] = ['name', 'type', 'children'];
  tableDataSource = new MatTableDataSource<EntityRow>([]);
  clickedRows = new Set<any>();
  tableDataStack: {rows: EntityRow[], selectedRow: EntityRow | null}[] = [];
  selectedEntity: EntityRow | null = null;
  entityCount: number = 0;

  onRowClick(row: EntityRow) {
    this.tableDataStack.push({rows: deepClone(this.tableDataSource.data), selectedRow: this.selectedEntity});
    this.selectedEntity = row;
  
    if (row.relations && row.relations.length) {
      const observables = row.relations.map(relation => {
        switch (relation.to.entityType) {
          case 'DEVICE':
            return this.ctx.deviceService.getDevice(relation.to.id).pipe(
              map(device => ({
                id: device.id.id,
                name: device.name,
                type: 'DEVICE',
          ...
          case 'ASSET':
            return this.ctx.assetService.getAsset(relation.to.id).pipe(
              map(asset => ({
                id: asset.id.id,
                name: asset.name,
                type: 'ASSET',
      ...
      forkJoin(observables).subscribe({
        next: (results) => {
          // Filter out null results and set as new table data
          const validResults = results.filter(result => result !== null);
          this.tableDataSource.data = [...validResults];
          
          // Load relations for new rows
          validResults.forEach(newRow => this.loadChildrenRows(newRow));
        },
        error: (err) => {
  ...
  goBack() {
    if (this.tableDataStack.length) {
      const previousData = this.tableDataStack.pop();
      this.tableDataSource.data = previousData.rows;
      this.selectedEntity = previousData.selectedRow;
    }
  }

  initializeEntityCount() {
    this.datasources.forEach(ds => {
      this.entityCount += 1;
      const relationQuery: EntityRelationsQuery = {
        parameters: {
          rootId: ds.entityId,
          rootType: ds.entityType,
          direction: 'FROM',
      ...
      } as unknown as EntityRelationsQuery;
  
      this.ctx.entityRelationService.findByQuery(relationQuery).subscribe({
        next: (relations) => {
          this.entityCount += relations.length;
  ...
  ngOnInit(): void {
    this.ctx.$scope.entitiesHierarchyProWidget = this;
  ...
  private initializeTableData() {
    const initialRows: EntityRow[] = this.datasources.map(ds => ({
      id: ds.entityId,
      name: ds.entityName,
      type: ds.entityType,
      relations: [] as EntityRelation[],
    }));
    this.tableDataSource.data = initialRows;
    initialRows.forEach(row => this.loadChildrenRows(row));
  }

  loadChildrenRows(row: any) {
    const relationQuery: EntityRelationsQuery = {
      parameters: {
        rootId: row.id,
        rootType: row.type,
